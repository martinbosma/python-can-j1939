...
class J1939_22:
    ...
    def send_pgn(self, data_page, pdu_format, pdu_specific, priority, src_address, data, time_limit, frame_format, tos=2, trailer_format=0):
        ...
        if data_length <= self.DataLength.TP:
            ...
            full_tp_size_packages = data_length // self.DataLength.TP
            full_payload_size = full_tp_size_packages * self.DataLength.TP
            data_list = [data[i:i + self.DataLength.TP] for i in range(0, full_payload_size, self.DataLength.TP)]
            if full_payload_size < data_length:
                data_list.append(data[full_payload_size:])
